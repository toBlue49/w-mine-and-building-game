...
@onready var ip_address: LineEdit = $MarginContainer/VBoxContainer/IP/LineEditIP
@onready var ip_port_host: LineEdit = $MarginContainer/VBoxContainer/HostMult/LineEdit
@onready var ip_port_join: LineEdit = $MarginContainer/VBoxContainer/IP/LineEdit
...
func _ready():
	...
	ip_port_host.text = global.config.get_value("settings", "ip_address_port_host", "9555")
	ip_port_join.text = global.config.get_value("settings", "ip_address_port_join", "9555")
...
func _host_mult_pressed() -> void:
	...
	global.config.set_value("settings", "player_name", global.player_name)
	global.config.set_value("settings", "size_box", size_box.value)
	global.config.set_value("settings", "ip_address_port_host", ip_port_host.text)
	global.config.save("user://data.cfg")
...
func _join_mult_pressed() -> void:
	...
	global.config.set_value("settings", "ip_address_port_join", ip_port_join.text)
	global.config.save("user://data.cfg")
	global.is_multiplayer = true
